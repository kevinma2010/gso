...
gsearch.prototype.parseResponse = function (body) {
    var arr = [];
    var doc = $(body);
    var search = doc.find("#search li[class='g']");
...
//            console.log("\n\n");
    ...
    if ($(body).find("#brs h3").html()) {
        var brs_cols, _brs_cols = []
        hasExtrares = true;
        extrares.title = $(body).find("#brs h3").html();
        brs_cols = $(body).find("div[class='brs_col']");
        for (var i = brs_cols.length - 1; i >= 0; i--) {
